...
def calorie_finder(input:str):
    max_calories_by_elf = []
    # sacks are separated by two newlines
    cleaned_input = input.split(sep="\n\n")
    # since they separate into strings with newlines
    # need to remove the newlines and turn into list of strings
    # then convert strings into ints to perform sum()
    for i in range(len(cleaned_input)):
        if "\n" in cleaned_input[i]:
            cleaned_input[i] = cleaned_input[i].split(sep='\n')
            cleaned_input[i] = [int(x) for x in cleaned_input[i]]
            max_calories_by_elf.append(sum(cleaned_input[i]))
        else:
            cleaned_input[i] = int(cleaned_input[i])
            max_calories_by_elf.append(cleaned_input[i])

    #cleaned_input[0].
    #print(max_calories_by_elf)
    #part1
    print(f"The max calories being held is: {max(max_calories_by_elf)}")

    #part2
    #find top three elves then get their sum
    sorted_maxes_by_elf = sorted(max_calories_by_elf) # very handy built-in sorting function lol
    top_three_sum = sum(sorted_maxes_by_elf[-3:]) # slice starts 3rd elf from the end
    print(f"The sum of the elves with the top three calories in their sacks is: {top_three_sum}")

calorie_finder(demo_input1)
